...
As mentioned, there are many open questions for which I will try to find explanations. In terms of practical applications, I will switch to Torch and Theano as these are more flexible and therefore enable to implement more sophisticated details. Currently I am also working on natural language processing with deep learning and in genreral would like to gain experiences with recurrent neural networks.